...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (jsonData.length >= 2) {
        var product = jsonData[1];

        var price = '$' + parseFloat(product.price).toFixed(2);
        var url = product.details_link;
        var imagePaths = [product.image_path2, product.image_path3].filter(function(path) {
            return path !== null;
        }).map(function(path) {
        ...
        var price1 = '$' + parseFloat(product1.price).toFixed(2);
        var url1 = product1.details_link;
        var imagePaths1 = [product1.image_path2, product1.image_path3].filter(function(path) {
            return path !== null;
        }).map(function(path) {
            if (path.startsWith('http://') || path.startsWith('https://')) {
                return path;
...
function showTooltip1(content1, imageUrls1, tooltipId1, pageUrl) {
    ...
    var imagePositions = [
        { left: '-5px', top: '-500px' }, // Vị trí của hình ảnh thứ nhất
